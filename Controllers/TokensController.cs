...
namespace API_GOPR_SERVICE.Controllers
{
    ...
    public class TokensController : ControllerBase
    {
        ...
        {
            var tokens = await _context.Token.FirstOrDefaultAsync(p => p.phoneNumber == phoneNumber);
        ...
        {
            var tokens = await _context.Token.FindAsync(id);
            if (tokens == null)
            {
                return NotFound();
            }

            _context.Token.Remove(tokens);
            await _context.SaveChangesAsync();
